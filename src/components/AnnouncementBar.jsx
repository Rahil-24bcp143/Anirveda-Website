...
const AnnouncementBar = () => {
  const announcements = [
    {
      id: 0,
      text: "🚀 Economania Registrations are NOW LIVE! Register for Breach 2026 and other exciting events!",
      priority: "high"
    },
    {
      id: 1,
  ...
  return (
    <>
      ...
      <div className="transition-all duration-500 ease-in-out">
        ...
        <div
          className={`bg-gradient-to-r from-primary via-secondary to-primary text-black transition-all duration-500 ease-out overflow-hidden shadow-lg border-b border-black/10 ${isVisible
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between h-full gap-2 sm:gap-3 md:gap-4">
            ...
            <div className="flex items-center space-x-1 sm:space-x-2 md:mr-4">
              ...
              <div className="md:hidden">
                <div className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full animate-pulse ${currentAnnouncement.priority === 'high' ? 'bg-red-500' : 'bg-green-500'
                  }`}></div>
              </div>
              ...
              <div className="hidden md:flex items-center space-x-2">
                {announcements.map((announcement, index) => (
                  <div
                    key={index}
                    className={`w-2.5 h-2.5 rounded-full transition-all duration-300 cursor-pointer hover:scale-125 transform ${index === currentIndex
            ...
            <div className="flex-1 min-w-0 flex items-center justify-center px-1 sm:px-2">
              <div className="flex items-center space-x-1 sm:space-x-2 w-full justify-center">
                ...
                <div className={`hidden md:block h-3 w-0.5 rounded-full animate-pulse ${currentAnnouncement.priority === 'high' ? 'bg-red-500' : 'bg-green-500'
                  }`}></div>
